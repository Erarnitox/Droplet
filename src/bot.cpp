...
auto start_bot() -> void {
	...
	bot.on_slashcommand([&bot, &global_command_list](const dpp::slashcommand_t& event) -> void {
		try {
			...
			bot.log(dpp::loglevel::ll_warning,
					fmt::format("Unhandeled exception occured in "
								"'handle_global_slash_commands' input was {}",
								event.command.msg.content));
	...
	bot.on_guild_member_add([&bot](const dpp::guild_member_add_t& event) -> void {
		bot.log(dpp::ll_info,
				fmt::format("A new member: {} joined the guild: {}", event.added.nickname, event.adding_guild->name));
	...
	bot.on_form_submit([&bot](const dpp::form_submit_t& event) -> void {
		try {
			...
			bot.log(dpp::loglevel::ll_info,
					fmt::format("[{}] form submitted with input: {}",
								event.custom_id,
								std::get<std::string>(event.components.at(0).components.at(0).value)));
		} catch (...) {
			bot.log(dpp::loglevel::ll_warning,
					fmt::format("Unhandeled exception occured in 'handle_form_submits', "
								"input was {}",
								std::get<std::string>(event.components.at(0).components.at(0).value)));
