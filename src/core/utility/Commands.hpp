...
typedef void (*handle_global_slash_commands)(const dpp::slashcommand_t& event,
											 dpp::cluster& bot,
...
static inline void call_register_global_slash_commands(register_global_slash_commands func,
													   std::vector<dpp::slashcommand>& command_list,
													   const dpp::cluster& bot) {
	func(command_list, bot);
}

static inline void call_handle_global_slash_commands(handle_global_slash_commands func,
...
static inline void call_welcome_member(welcome_member func, const dpp::guild_member_add_t& event, dpp::cluster& bot) {
	func(event, bot);
}

static inline void call_leave_member(leave_member func, const dpp::guild_member_remove_t& event, dpp::cluster& bot) {
	func(event, bot);
}

static inline void call_handle_button_clicks(handle_button_clicks func,
											 const dpp::button_click_t& event,
											 dpp::cluster& bot) {
	func(event, bot);
}

static inline void call_handle_form_submits(handle_form_submits func,
											const dpp::form_submit_t& event,
											dpp::cluster& bot) {
	func(event, bot);
}

static inline void call_handle_reaction_added(handle_reaction_added func,
											  const dpp::message_reaction_add_t& event,
											  dpp::cluster& bot) {
	func(event, bot);
}

static inline void call_handle_reaction_removed(handle_reaction_removed func,
