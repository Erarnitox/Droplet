...
std::optional<size_t> UserRepository::try_adjust_exp(size_t user_id, std::int64_t delta, size_t min_exp) noexcept {
	const static std::string sql_string{
		"UPDATE usr SET exp = exp + $2::int8 "
		"WHERE user_id = $1::int8 AND exp >= $3::int8 "
		"RETURNING exp"};

	if (not executor_.hasConnection() || user_id == 0) {
		return std::nullopt;
	}

	const auto rows{executor_.execSelectAll(sql_string, user_id, delta, min_exp)};
	if (rows.empty()) {
		return std::nullopt;
	}
	return rows.front().get<size_t>("exp");
}