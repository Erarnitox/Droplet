...
bool PortalRepository::update(const PortalDTO& object) noexcept {
	const static std::string sql_string{"UPDATE portals SET channel_id = $1::int8 WHERE guild_id = $2::int8"};
	...
	return executor_.execQuery(sql_string, object.channel_id, object.guild_id);
}
