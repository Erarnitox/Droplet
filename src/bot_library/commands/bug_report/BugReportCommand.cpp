...
void BugReportCommand::on_slashcommand(const dpp::slashcommand_t& event) {
  ...
  modal.add_component(
      dpp::component().set_type(dpp::cot_text).set_label("Short description")
      .set_id(FIELD_DESC).set_placeholder("What is the bug?")
      .set_text_style(dpp::text_short).set_required(true)
  );

  modal.add_row();

  modal.add_component(
      dpp::component().set_type(dpp::cot_text).set_label("Steps to Reproduce")
      .set_id(FIELD_STEPS).set_placeholder("How to trigger the bug?")
      .set_text_style(dpp::text_paragraph).set_required(true)
  );

  modal.add_row();

  modal.add_component(
      dpp::component().set_type(dpp::cot_text).set_label("Expected Behavior")
      .set_id(FIELD_EXPECTED).set_placeholder("What should have happened?")
      .set_text_style(dpp::text_paragraph).set_required(true)
  );
...
void BugReportCommand::on_form_submit(const dpp::form_submit_t& event) {
  ...
  Bot::ctx->direct_message_create(ERARNITOX_ID, dpp::message(message),
      [event](const dpp::confirmation_callback_t& cc) {
        if (not cc.is_error()) {
            Core::timed_reply_private(*Bot::ctx, event, "Thanks for your report!", 3000);
        } else {
            Core::timed_reply_private(*Bot::ctx, event, "Sending failed! Please contact @erarnitox directly!", 5000);
        }
    });
}
