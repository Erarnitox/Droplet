...
class ChallengeBadgeCommand : public IGlobalSlashCommand, public IButtonCommand, public IFormCommand {
  public:
	static constexpr std::string_view k_name{"challenge_badge"};
	static constexpr std::string_view k_description{"Create challenge Badge (Admin only!)"};

	explicit ChallengeBadgeCommand(AppContext& ctx);
