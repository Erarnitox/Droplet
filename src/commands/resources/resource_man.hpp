...
auto register_global_slash_commands(std::vector<dpp::slashcommand> &command_list, const dpp::cluster &bot) -> void {
	...
	// action option
	res_command.add_option(dpp::command_option(dpp::co_string, "action", "What would you like to do?", true)
	...
	// title option
	res_command.add_option(dpp::command_option(dpp::co_string, "title", "The title of the resource", false));

	// query option
	res_command.add_option(dpp::command_option(dpp::co_string, "query", "what to search for", false));

	// index option
	res_command.add_option(dpp::command_option(dpp::co_string, "index", "The index of the resource", false));

	// link option
	res_command.add_option(dpp::command_option(dpp::co_string, "link", "A link to the resource", false));
