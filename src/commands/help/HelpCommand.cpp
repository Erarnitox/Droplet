...
auto HelpCommand::registerGlobalSlashCommand(std::vector<dpp::slashcommand> &command_list, const dpp::cluster &bot) noexcept -> void {
    dpp::slashcommand help_command("help", "Usage information", bot.me.id);
    command_list.push_back(help_command);
}

auto HelpCommand::handleGlobalSlashCommand(const dpp::slashcommand_t& event, dpp::cluster& bot, const std::vector<dpp::slashcommand>& command_list) noexcept -> void {
    // unneeded arguments:
    (void) command_list;

    if (event.command.get_command_name() != "help") return;
