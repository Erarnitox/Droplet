...
bool ChallengeRoleRepository::create(const ChallengeRoleDTO& object) {
    static std::string sql_string{ "INSERT INTO challenge_roles(role_id, guild_id, message_id, flag) VALUES ($1::int8, $2::int8, $3::int8, $4::varchar)" };
    
    if(!Database::hasConnection()) return false;
    if(!object.messageId) return false;
    if(this->get(object.messageId).guildId != 0) return false;
...
bool ChallengeRoleRepository::update(const ChallengeRoleDTO& object) {
    static std::string sql_string{ "UPDATE challenge_roles(role_id, guild_id, message_id, flag) VALUES ($1::int8, $2::Int8, $3::int8, $4::varchar) WHERE message_id = $3" };
