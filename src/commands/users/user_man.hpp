...
auto register_global_slash_commands(std::vector<dpp::slashcommand> &command_list, const dpp::cluster &bot) -> void {
	...
	help_command.add_option(dpp::command_option(dpp::co_string, "user", "User you would like information about", true));

	// command_list.push_back(help_command);
}

static inline auto handle_global_slash_commands(const dpp::slashcommand_t &event, dpp::cluster &bot) -> void {
	if (event.command.get_command_name() == "user_info") {
		auto mentioned_user{Core::getParameter(bot, event, "user")};
		if (mentioned_user.empty())
			return;

		// const dpp::guild_member& member{
		// event.command.get_guild().members.find(mentioned_user) };
...
static inline auto welcome_member(const dpp::guild_member_add_t &event, dpp::cluster &bot) -> void {
	...
	dpp::embed embed{
		dpp::embed()
			...
			.set_description(std::string("Welcome ") + event.added.get_mention() + " to " + event.adding_guild->name)
